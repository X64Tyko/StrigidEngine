#include "TestFramework.h"
#include "TrinyxEngine.h"
#include "FieldProxy.h"

// Validates FieldProxy<float, Scalar> — the single-entity scalar update path.
// This is the most commonly exercised path (Construct-side entity writes).
// Tests: store via operator=, load via Value(), dirty bit marking, Advance() cursor.
TEST (FieldProxy_ScalarStoreLoad)
{
	(void)Engine;

	// Raw backing arrays — simulate what BuildFieldArrayTable provides
	alignas(FIELD_ARRAY_ALIGNMENT) SimFloat data[16] = {};
	alignas(FIELD_ARRAY_ALIGNMENT) int32_t flags[16] = {};

	// --- Basic store and load ---
	{
		FieldProxy<SimFloat, FieldWidth::Scalar> proxy;
		proxy.Bind(data, flags, 0);

		proxy = SimFloat(3.14f);
		ASSERT_EQ(proxy.Value(), SimFloat(3.14f));
		ASSERT_EQ(data[0], SimFloat(3.14f));
	}

	// --- Dirty bit set on store ---
	{
		FieldProxy<SimFloat, FieldWidth::Scalar> proxy;
		proxy.Bind(data, flags, 0);
		flags[0] = 0; // clear first

		proxy = SimFloat(1.0f);

		constexpr int32_t DirtyBit        = static_cast<int32_t>(1u << 30);
		constexpr int32_t DirtiedFrameBit = static_cast<int32_t>(1u << 29);
		ASSERT((flags[0] & DirtyBit) != 0);
		ASSERT((flags[0] & DirtiedFrameBit) != 0);
	}

	// --- Advance cursor ---
	{
		data[0] = SimFloat(10.0f);
		data[1] = SimFloat(20.0f);
		data[2] = SimFloat(30.0f);

		FieldProxy<SimFloat, FieldWidth::Scalar> proxy;
		proxy.Bind(data, flags, 0);
		ASSERT_EQ(proxy.Value(), SimFloat(10.0f));

		proxy.Advance(1);
		ASSERT_EQ(proxy.Value(), SimFloat(20.0f));

		proxy.Advance(1);
		ASSERT_EQ(proxy.Value(), SimFloat(30.0f));
	}

	// --- Compound assignment operators ---
	{
		FieldProxy<SimFloat, FieldWidth::Scalar> proxy;
		proxy.Bind(data, flags, 0);
		data[0] = SimFloat(10.0f);

		proxy += SimFloat(5.0f);
		ASSERT_EQ(proxy.Value(), SimFloat(15.0f));

		proxy -= SimFloat(3.0f);
		ASSERT_EQ(proxy.Value(), SimFloat(12.0f));

		proxy *= SimFloat(2.0f);
		ASSERT_EQ(proxy.Value(), SimFloat(24.0f));

		proxy /= SimFloat(4.0f);
		ASSERT_EQ(proxy.Value(), SimFloat(6.0f));
	}

	// --- Comparison operators (no dirty marking) ---
	{
		FieldProxy<SimFloat, FieldWidth::Scalar> proxy;
		proxy.Bind(data, flags, 0);
		data[0]  = SimFloat(5.0f);
		flags[0] = 0;

		// Comparisons should not mark dirty
		[[maybe_unused]] auto gt = proxy > SimFloat(4.0f);
		[[maybe_unused]] auto lt = proxy < SimFloat(6.0f);
		[[maybe_unused]] auto eq = proxy == SimFloat(5.0f);
		ASSERT_EQ(flags[0], 0); // no dirty bits set by comparisons
	}
}