...
TEST (FieldProxy_WideStore)
{
	...
	// --- Wide store: all 8 lanes written unconditionally ---
	{
		...
		proxy = SimFloat(7.0f); // broadcasts scalar to all 8 lanes

		for (int i = 0; i < 8; ++i) ASSERT_EQ(data[i], 7.0f);
	}
	...
	{
		alignas(FIELD_ARRAY_ALIGNMENT) SimFloat partial[8] = {0, 0, 0, 0, 0, 0, 0, 0};
		FieldProxy<SimFloat, FieldWidth::Wide> proxy;
		...
		for (int i = 0; i < 8; ++i) ASSERT_EQ(partial[i], 3.0f);
	}

	// --- Wide +=, with dirty bits set on all 8 lanes ---
	{
		for (int i = 0; i < 8; ++i) data[i] = static_cast<SimFloat>(i);
		...
		for (int i = 0; i < 8; ++i) ASSERT_EQ(data[i], static_cast<SimFloat>(i) + 10.0f);

		constexpr int32_t DirtyBit = static_cast<int32_t>(1u << 30);
		for (int i = 0; i < 8; ++i) ASSERT((flags[i] & DirtyBit) != 0);
	}
#endif
}